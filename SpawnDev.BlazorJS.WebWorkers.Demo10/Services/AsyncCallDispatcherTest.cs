namespace SpawnDev.BlazorJS.WebWorkers.Demo10.Services
{
    ...
    public class AsyncCallDispatcherTest : IAsyncCallDispatcherTest
    {
        ...
        public async Task Test(AsyncCallDispatcher caller)
        {
            ...
            var IITestProp1ValueToSet = "Yay!";
            await caller.Set<AsyncCallDispatcherTest, string>(s => s.IITestProp1, IITestProp1ValueToSet);
            // instance property get
            Console.WriteLine("Expression instance properties get");
            var IITestProp1Value = await caller.Run<AsyncCallDispatcherTest, string>(s => s.IITestProp1);
            if (IITestProp1Value != IITestProp1ValueToSet)
            {
                throw new Exception("Set then get failed");
            }
            // instance methods
